...
import sequ.biz.appmoduleqrcode.R;
...
public final class ViewfinderView extends View {
	 ...
	 * 扫描框中的中间线的与扫描框左右的间隙
	 */
	private static float	        MIDDLE_LINE_PADDING;
	...
	public ViewfinderView(Context context, AttributeSet attrs) {
		...
		// 将像素转换成dp
		MIDDLE_LINE_PADDING = ScreenRate = (int) (20 * density);
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			frameHeight = (frameHeight + 1) % frame.height();
			// int middle = frame.height() / 2 + frame.top;
			// int middle = frame.top + frameHeight;
			int middle = frame.height() / 2 + frame.top;
			canvas.drawRect(frame.left + MIDDLE_LINE_PADDING, middle - 1, frame.right - MIDDLE_LINE_PADDING,
			        middle + 2, paint);
